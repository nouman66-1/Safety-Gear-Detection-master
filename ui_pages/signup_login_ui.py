...
class UI:
    ...
    def create_login_frame(self):
        ...
        self.username_entry.grid(row=1, column=0, columnspan=2, pady=1, padx=10, sticky="nsew")

        self.password_entry = ctk.CTkEntry(self.login_frame, placeholder_text="Password", show="*", width=200)
        self.password_entry.grid(row=2, column=0, columnspan=2, pady=1, padx=10, sticky="nsew")
    ...
    def login(self):
        ...
        if user and user.password == password:
            if user.is_admin:
                ...
                if user.total_violation_count >= 100:
                    CustomMessageBox(
    ...
    def create_logout_frame_for_admin(self):
        ...
        for row, user in enumerate(user_list, start=1):
            ...
            total_violation_count = f"{user.total_violation_count} / 100"
            user_type_label = ctk.CTkLabel(
                self.table_frame, text=total_violation_count, text_color="black", fg_color="red" if user.total_violation_count >= 100 else "white"
            )
    ...
    def create_logout_frame_for_employee(self):
        ...
        if user.is_boots_check.lower() == "active":
            self.gear_status_labels["Boots"] = ctk.CTkLabel(self.result_label, text="Boots: No", font=("Arial", 12), fg_color="red")
